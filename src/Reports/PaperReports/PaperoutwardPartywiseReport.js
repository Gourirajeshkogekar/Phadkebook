...
function PaperoutwardPartywiseReport() {
  ...
  //       doc.setTextColor(0);
  //       doc.setFontSize(10);
  //       doc.text(`Page ${pageNum}`, doc.internal.pageSize.getWidth() - 20, 10);
  //     };
  ...
  //     for (const [partyName, parties] of Object.entries(parties)) {
  //       doc.setTextColor(0);
  ...
  //       for (const [paperType, entries] of Object.entries(groupedData)) {
  //         doc.setFont(undefined, "bold");
  //         doc.setFontSize(11);
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      // Step 1: Group by PaperSizeName > AccountName
      const groupedData = data.reduce((acc, row) => {
        const paper = row.PaperSizeName;
        const party = row.AccountName;
        if (!acc[paper]) acc[paper] = {};
      ...
      // Step 2: Invert grouping to AccountName > PaperSizeName
      const partywiseData = Object.entries(groupedData).reduce(
        (acc, [paper, parties]) => {
          for (const [party, entries] of Object.entries(parties)) {
            if (!acc[party]) acc[party] = {};
            acc[party][paper] = entries;
          }
          return acc;
        },
        {}
      ...
      for (const [partyName, paperMap] of Object.entries(partywiseData)) {
        ...
        for (const [paperType, entries] of Object.entries(paperMap)) {
          doc.setFont(undefined, "bold");
          doc.setFontSize(11);
          doc.text(paperType, pageWidth / 2, y, { align: "center" });
          y += 6;
          ...
          doc.rect(colX, y, cols[6].width, rowHeight); // Unit column (blank)
          y += rowHeight + 6;
...
export default PaperoutwardPartywiseReport;
