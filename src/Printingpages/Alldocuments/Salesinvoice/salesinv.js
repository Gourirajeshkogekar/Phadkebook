...
function SalesInvoice() {
  ...
  const handleShowPreview = async () => {
    ...
    try {
      const response = await axios.get(
        "https://publication.microtechsolutions.net.in/php/get/getSalesInvoice.php",
        {
          params: {
            startNumber: startNo,
            endNumber: endNo,
            allChallansRequired: allchallansreqd,
            printCanvassorsBill: printcanvassorsbill,
          },
        }
      );
      ...
      const mappedData = rows.map((item) => {
        ...
        const rate = Number(item.Rate.replace(/,/g, ""));

        return {
          BookCode: `P ${item.Code}`,
          StandardName: item.Class,
          BookName: item.Description,
          Copies: Number(item.Qty),
          Price: rate,
          Amount: amount,
          Discount: 30,
          DiscountAmount: amount - amount * 0.3,
        };
      });
  ...
  if (showPreview) {
    return (
      ...
      >
        ...
        <Box sx={{ mt: 4, width: "210mm", bgcolor: "white" }}>
          <SalesInvoicePrint ref={reportRef} data={apiData} />
        </Box>
      </Box>
    );
  }
  ...
  return (
    ...
    >
      <Paper sx={{ width: "100%", maxWidth: 450 }}>
        <Box
          sx={{
            bgcolor: "#1e293b",
            p: 2,
        ...
        <Box sx={{ p: 4 }}>
          <Box display="flex" flexDirection="column" gap={3}>
            <Box display="flex" justifyContent="space-between">
              <Typography fontWeight="bold">Start Number:</Typography>
            ...
            <Box display="flex" justifyContent="space-between">
              <Typography fontWeight="bold">End Number:</Typography>
            ...
            <FormControlLabel
              control={
                <Checkbox
                  checked={allchallansreqd}
                  onChange={(e) =>
                    setAllchallansreqd(e.target.checked)
                  }
                />
              }
              label="All Challans Required"
            />
            ...
            <FormControlLabel
              control={
                <Checkbox
                  checked={printcanvassorsbill}
                  onChange={(e) =>
                    setPrintcanvassorsbill(e.target.checked)
                  }
                />
              }
