...
export default function TDSCertificatePrint({
  ...
  const header = safeRows.length > 0 ? safeRows[0] : {};

  const nature = header["Nature of Payment"] || "";
  const section = header["Section"] || "";
  const payeePAN = header["PAN/GIR No. of Payee"] || "";
  const deductorPAN = header["Deductor PAN/GIR"] || "";
  const period = header["Financial Period"] || `${startDate} To ${endDate}`;
  ...
  return (
    ...
    >
      ...
      <table width="100%" style={{ borderCollapse: "collapse", marginTop: 10 }} border="1">

        <tbody>

          <tr>
            <td width="33%">
              <b>Name & address of the person deducting tax</b>
              <br/><br/>
              <b>Phadke Prakashan, Kolhapur.</b>
              <br/>
              Phadke Bhavan, Near Hari Mandir,
              <br/>
              Dudhali Kolhapur 416012
            </td>

            <td width="33%">
              <b>Acknowledgement Nos. of statements</b>

              <table width="100%" border="1" style={{ borderCollapse:"collapse", marginTop:5 }}>
                <thead>
                  <tr>
                    <th>Quarter</th>
            ...
            <td width="33%">
              <b>Name & address of the person to whom payment made</b>
              <br/><br/>
              <b>{party}</b>
            </td>
          ...
          <tr>
            ...
            <td>
              <b>TAX DEDUCTION A/C OF THE DEDUCTOR</b>
              <br/>
              KLPMO 1647 C
            </td>
          ...
          <tr>
            ...
            <td>
              <b>PAN / GIR NO. OF THE DEDUCTOR</b>
              <br/>
              {deductorPAN}
            </td>
      ...
      <table width="100%" border="1" style={{ borderCollapse:"collapse", marginTop:5 }}>
        ...
        <tbody>
          ...
          {safeRows.map((r,i)=>{
            ...
            const isTotal = r["Date of Payment/Credited"] === "**TOTAL**";

            return(
              <tr key={i} style={isTotal ? {fontWeight:"bold"} : {}}>

                <td>{r["Sr. No."]}</td>
                <td>{r["Amount Paid/Credited"]}</td>
                <td>{r["Date of Payment/Credited"]}</td>
                <td>{r["TDS Rs."]}</td>
                <td>{r["TDS Surcharge"]}</td>
                <td>{r["Edu. Cess"]}</td>
                <td>{r["S&HEdu Cess"]}</td>
                <td>{r["Total Tax Deposited"]}</td>
                <td>{r["Cheque/DD No."]}</td>
                <td>{r["BSR Code"]}</td>
                <td>{r["Date Tax Deposited"]}</td>
                <td>{r["Transfer Voucher"]}</td>
