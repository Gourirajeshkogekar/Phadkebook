...
function PartyCategory() {
  ...
  const handleEdit = (row) => {
    const mode = partycategories[row.index];
    setPartyCategory(mode.PartyCategory); // Ensure this matches your data field
  ...
  const confirmDelete = () => {
    ...
    fetch(
      "https://publication.microtechsolutions.net.in/php/PartyCategorydelete.php",
      requestOptions
    )
      .then((response) => response.text())
      .then((result) => console.log(result))
      .catch((error) => console.error(error));
    toast.success("Party category Deleted Successfully");
  ...
  const validateForm = () => {
    ...
    if (!PartyCategory) {
      formErrors.PartyCategory = "Party Category is required.";
      isValid = false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const url = isEditing
      ? "https://publication.microtechsolutions.net.in/php/PartyCategoryupdate.php"
      : "https://publication.microtechsolutions.net.in/php/PartyCategorypost.php";
    ...
    try {
      ...
      if (isEditing) {
        toast.success("Party Category  updated successfully!");
      } else {
        toast.success("Party Category added successfully!");
      }
      setIsModalOpen(false);
      resetForm();
      fetchPartycategories();
        ...
        <Modal open={isModalOpen}>
          <div className="partycat-modal">
            <h2
              ...
              {editingIndex >= 0 ? "Edit Party category" : "Add Party category"}
            </h2>
            <form onSubmit={handleSubmit} className="partycat-form">
              <div>
                <label className="partycat-label">Party Category Name</label>
                <div>
                  <input
                    type="text"
                    id="PartyCategory"
